        ...
        virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const 
{
    ...
    residuals[0] = parameters[0][0] + parameters[0][1] + parameters[1][0] + parameters[1][1] ; 
    if(jacobians )
    {

	if(jacobians[0]) // dy2/dTi
	{
	    Eigen::Map<Eigen::Matrix<double, 1, 2, Eigen::RowMajor> > jacobian_pose_i(jacobians[0]); 
	...
	{
	    Eigen::Map<Eigen::Matrix<double, 1, 2, Eigen::RowMajor> > jacobian_pose_i(jacobians[1]); 
...
//ODkomentiraj
        ...
        J = 0.5*( X.transpose()* (this->H_ + this->H_.transpose() ) ) + this->f_.transpose();
...
// //       new SizedCostFunction<1, num_variables> (cost2);
  ...
  problem.AddResidualBlock(cost1, NULL, x, x3);
  //problem.AddResidualBlock(cost2, NULL, x1);

for(int i = 0; i< 4; i++){

problem.SetParameterLowerBound(x,0, lower_bounds(1,0)+i*10);
  problem.SetParameterUpperBound(x,0, upper_bounds(1,0)+i*10);
